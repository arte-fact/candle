...
pub mod wav;
use candle::utils::{cuda_is_available, hip_is_available, metal_is_available};
...
pub fn device(cpu: bool) -> Result<Device> {
    if cpu {
        ...
        {
            println!("Running on CPU, to run on GPU, build this example with `--features cuda` or `--features hip`");
        }
        Ok(Device::Cpu)
    }
}
