...
// Use f32 promotion for all math ops to be safe across all GCN/RDNA archs.
__device__ __forceinline__ __half powg(__half a, __half b) { return __float2half(powf(__half2float(a), __half2float(b))); }
__device__ __forceinline__ bool isnang(__half a) { return isnan(__half2float(a)); }
__device__ __forceinline__ __half sqrtg(__half a) { return __float2half(sqrtf(__half2float(a))); }
__device__ __forceinline__ __half cosg(__half a) { return __float2half(cosf(__half2float(a))); }
__device__ __forceinline__ __half sing(__half a) { return __float2half(sinf(__half2float(a))); }
__device__ __forceinline__ __half recipg(__half a) { return __float2half(1.0f / __half2float(a)); }
__device__ __forceinline__ __half maxg(__half a, __half b) { return candle_hmax_nan(a, b); }
...
__device__ __forceinline__ __half ming(__half a, __half b) { return candle_hmin_nan(a, b); }
__device__ __forceinline__ __half logg(__half a) { return __float2half(logf(__half2float(a))); }
__device__ __forceinline__ __half expg(__half a) { return __float2half(expf(__half2float(a))); }
__device__ __forceinline__ __half absg(__half a) { return __float2half(fabsf(__half2float(a))); }
...
// bf16 -- always available on HIP via hip_bfloat16 (emulated through f32 conversions)
...
// fp8 (e4m3) -- not supported on AMD gfx906, guarded out
